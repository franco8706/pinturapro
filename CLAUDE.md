...
- **Agentes y kit (reutilizables desde cualquier sesión):** cuatro agentes definidos en `.claude/agents/` — `recorrido-web`, `formularios-hostiles`, `simulador-color`, `seguridad-rls` — con modelo y herramientas acotadas. El kit vive en **`tools/auditoria/`** (dentro del repo: antes estaba en `/tmp` y el Codespace se lo llevó puesto dos veces): `navegador.cjs` (Chrome propio por agente, `auditar()` estándar, espera de hidratación), `REGLAS.md`, `generar.py` (fotos de prueba) y `mascara.py` (máscaras de referencia del simulador). Se los llama por nombre y ya saben el contexto: no hay que volver a explicárselo en cada sesión.
- **Arquitectura — el proyecto deja de ser un bloque único** (decisión del dueño): ver `docs/arquitectura.md`. Primer corte hecho: **`packages/color`** (varita mágica + OKLab), que `apps/web` consume como dependencia del workspace. Siguen `packages/dominio` (estados de trabajo, comisión, permisos, textos de error) y `packages/datos` (queries y mutaciones de Supabase), que hoy están duplicados entre web y móvil.
