...
- **Auditoría (19 sept 2026, agentes con navegador propio):** se corrigieron tres cosas medidas en Chrome, no deducidas:
  ...
  · Kit de agentes en `/workspaces/codespaces-blank/.auditoria/kit/` (fuera de `/tmp`, que el Codespace borra al reiniciar): `navegador.cjs` (Chrome propio por agente, `auditar()` estándar, espera de hidratación) y `REGLAS.md`. Fotos de prueba y máscaras de referencia en `.auditoria/fotos/`.
