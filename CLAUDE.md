...
- **App móvil (`apps/mobile/`, Expo + React Native + TypeScript):** mismo Supabase que la web (anon key, RLS). Stack: expo-router, supabase-js + AsyncStorage (sesión persistente), theme de constantes que replica los tokens (sin NativeWind, StyleSheet). Hecho: tabs Pintores/Trabajos/Cuenta, lista de pintores (datos reales), detalle de pintor (rating/bio/pros/cons/portfolio), trabajos abiertos, login email/contraseña. Capa de datos en `apps/mobile/lib/queries.ts` (espejo de la web con supabase-js). Correr: `cd apps/mobile && npm install && npx expo install --fix && npx expo start` (Expo Go). No se puede previsualizar desde el Codespace. Pendiente: cotizar/publicar desde la app, perfil del pintor, contenido, OAuth con deep links, push, EAS Build. Decisión de infra/contenedores en `docs/infraestructura.md`; plan móvil en `docs/mobile-plan.md`.
